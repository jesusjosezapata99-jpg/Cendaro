#!/usr/bin/env node
/**
 * T1.13 — design guard (PLAN-2026-09-DESIGN-SYSTEM §F1).
 *
...
// Public landing and login pages are explicitly scoped for Phase F9
// (PLAN-2026-09-DESIGN-SYSTEM §F9: T9.1 & T9.2).
const IS_F9_FILE = (filePath) => {
  const norm = filePath.replace(/\\/g, "/");
  return norm.includes("app/_components/landing/") || norm.includes("app/login/");
};
...
async function countPattern(pattern) {
  ...
  for (const file of files) {
    if (pattern.deferF9Files && phaseNum < 9 && IS_F9_FILE(file)) {
      continue;
    }
