...
export function Hero() {
  ...
  return (
    <section className="relative flex min-h-[90dvh] flex-col items-center justify-center overflow-hidden px-6 pt-32 pb-24 text-center">
      {/* Trust badge — pill allowed per spec */}
      <m.div
      ...
      {/* Subtitle */}
      <m.p
        className="text-muted-foreground mx-auto mt-6 max-w-xl text-lg font-normal"
        initial={{ opacity: 0, y: 24, filter: "blur(10px)" }}
        animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
      ...
      <m.span
        className="text-muted-foreground mt-3 text-sm font-normal"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      {/* Product visual capture / video demo with 1px border treatment */}
      <m.div
