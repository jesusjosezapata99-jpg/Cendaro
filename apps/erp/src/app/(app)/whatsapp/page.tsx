...
const STATUS_CFG: Record<string, { label: string; color: string }> = {
  ...
  dispatched: { label: "Despachado", color: "bg-indigo-500/20 text-indigo-400" },
  cancelled: { label: "Cancelado", color: "bg-red-500/20 text-red-400" },
  returned: { label: "Devuelto", color: "bg-slate-500/20 text-muted-foreground" },
};
...
export default function WhatsAppPage() {
  ...
  const todayOrders = waOrders.filter((o) => o.createdAt.toString().startsWith(todayStr)).length;
    ...
    <div className="p-4 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Canal de venta asistida</p>
        </div>
        <Link
          href="/orders"
          className="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-emerald-500"
        >
          + Registrar Venta
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        {[
          { label: "Pedidos Hoy", value: todayOrders, icon: "📋", accent: "border-emerald-500/40" },
          { label: "Ingresos Hoy", value: `$${todayRevenue.toFixed(2)}`, icon: "attach_money", accent: "border-blue-500/40" },
          { label: "Total Pedidos WA", value: waOrders.length, icon: "chat", accent: "border-green-500/40" },
          { label: "Pendientes", value: waOrders.filter((o) => o.status === "pending").length, icon: "⏳", accent: "border-amber-500/40" },
        ].map((stat) => (
      ...
      <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
        <div className="flex items-center gap-2">
          <span>ℹ️</span>
          <div>
            <p className="text-sm font-medium text-emerald-400">Canal híbrido: WhatsApp consume stock de tienda</p>
            <p className="text-xs text-emerald-400/60">Las ventas se registran manualmente. Operación ligera.</p>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
              ...
              {waOrders.map((o) => {
                ...
                return (
                  <tr key={o.id} className="border-b border-border transition-colors hover:bg-accent/50">
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{o.customerId?.slice(0, 8) ?? "—"}</td>
                    <td className="px-4 py-3 text-right font-mono font-bold text-foreground">${Number(o.total).toFixed(2)}</td>
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card py-12 text-muted-foreground">
          ...
          <p className="mt-1 text-xs text-muted-foreground/60">Las ventas de este canal se registran desde Pedidos</p>
        </div>
      )}
    </div>
  );
