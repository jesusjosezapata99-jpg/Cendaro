 ...
 *
 * Wrapped in <Delayed> (NN/g guidance): navigations that resolve
 * quickly never flash a skeleton — the placeholder only appears
 * if the segment actually takes >200 ms.
 */
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="border-border bg-card border p-4">
              <div className="bg-muted mb-2 h-3 w-20 animate-pulse" />
        ...
        <div className="border-border bg-card border p-4">
          <div className="space-y-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="bg-muted h-10 w-full animate-pulse" />
            ))}
          </div>
        </div>
      </div>
