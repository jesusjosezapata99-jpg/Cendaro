...
const CreateContainerDialog = lazy(() => import("~/components/forms/create-container").then(m => ({ default: m.CreateContainerDialog })));
...
export default function ContainersPage() {
  ...
  const { data: containers, isLoading } = useQuery(trpc.container.list.queryOptions());
  ...
  return (
    ...
    <div className="p-4 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-foreground">Contenedores</h1>
          <p className="text-sm text-muted-foreground mt-1">Gestión de importaciones y recepción de mercancía</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        {[
          { label: "Total", value: isLoading ? "—" : list.length, icon: "package_2", accent: "border-blue-500/40" },
          { label: "En Tránsito", value: isLoading ? "—" : inTransit, icon: "directions_boat", accent: "border-cyan-500/40" },
          { label: "Pendientes", value: isLoading ? "—" : pending, icon: "move_to_inbox", accent: "border-amber-500/40" },
          { label: "FOB Total", value: isLoading ? "—" : `$${totalFob.toLocaleString()}`, icon: "attach_money", accent: "border-emerald-500/40" },
        ].map((stat) => (
      ...
      <div className="flex gap-2">
        {["all", "created", "in_transit", "received", "closed"].map((status) => (
          <button
            key={status}
      ...
      {isLoading ? (
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-2xl text-muted-foreground">{cfg.icon}</span>
                    <div>
                      <h3 className="font-mono text-lg font-bold text-foreground">{container.containerNumber}</h3>
                    </div>
                  </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Salida</p>
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Llegada Est.</p>
                    <p className="text-sm text-foreground">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Costo FOB</p>
        ...
        <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
          <span className="material-symbols-outlined text-4xl mb-2">package_2</span>
