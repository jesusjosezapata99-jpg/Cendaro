...
import ContainerDetailClient from "./client";
...
/**
 * Container Detail — Server Component with SSR Prefetch.
...
export default async function ContainerDetailPage({
  ...
  return (
    <Suspense fallback={<AppLoading />}>
      <ContainerDetailPrefetch id={id} />
    </Suspense>
  );
}

async function ContainerDetailPrefetch({ id }: { id: string }) {
  const queryClient = getQueryClient();

  try {
    await queryClient.prefetchQuery(trpc.container.byId.queryOptions({ id }));
  } catch {
