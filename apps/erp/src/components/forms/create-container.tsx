...
export function CreateContainerDialog({ open, onClose }: Props) {
  ...
  const create = useMutation(
    trpc.container.create.mutationOptions({
      onSuccess: () => {
        ...
        setForm({ containerNumber: "", departureDate: "", arrivalDate: "", costFob: "", notes: "" });
        onClose();
      },
    }),
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    create.mutate({
      ...
      arrivalDate: form.arrivalDate ? new Date(form.arrivalDate).toISOString() : undefined,
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Nuevo Contenedor" description="Registra un nuevo contenedor de importación">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Field label="Costo FOB (USD)" hint="Costo total del contenedor en dólares">
          <Input
            type="number"
            step="0.01"
            min="0"
            placeholder="0.00"
            value={form.costFob}
            onChange={(e) => setForm((f) => ({ ...f, costFob: e.target.value }))}
          />
        </Field>
        <Field label="Notas">
          <TextArea
        ...
        <FormActions onCancel={onClose} submitting={create.isPending} submitLabel="Crear Contenedor" />
      </form>
    </Dialog>
  );
}
