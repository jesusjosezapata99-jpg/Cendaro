  ...
  const update = useMutation(
    trpc.sales.updateOrderStatus.mutationOptions({
      onMutate: async (variables) => {
        ...
        const queryKey = trpc.sales.orderById.queryKey({ id: variables.id });
        const previousOrder = qc.getQueryData(queryKey);
        if (previousOrder) {
      ...
      onError: (err, _variables, context) => {
        if (context?.queryKey && context.previousOrder) {
          qc.setQueryData(context.queryKey, context.previousOrder);
        }
