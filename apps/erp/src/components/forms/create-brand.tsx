...
export function CreateBrandDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  const trpc = useTRPC();
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Nueva Marca">
      <form
        onSubmit={(e) => { e.preventDefault(); create.mutate({ name, slug, description: description || undefined }); }}
        className="space-y-4"
      >
        <div>
        ...
        <div>
          ...
          <input value={description} onChange={(e) => setDescription(e.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
        </div>
