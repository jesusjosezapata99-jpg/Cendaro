...
/**
 * Notification center — PLAN-2026-09-DESIGN-SYSTEM §5.8.1, M-25 (T2.6).
 * Replaces `notifications-dropdown.tsx` with the spec'd Popover
 * (`h-133.75 w-screen md:w-100 p-0`, `align="end" sideOffset={10}`).
 *
 * The trigger's `activeAlertCount` query is deferred 500ms after mount
 * (Deferred connection status pattern) so it doesn't join the page's
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="end"
        sideOffset={10}
        className="flex h-133.75 w-screen flex-col p-0 md:w-100"
      >
